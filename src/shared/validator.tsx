  ...
  const validator = (
    ...
    rules.forEach(r => {
      ...
      if (isEmpty(value) && r.required) {
        addErrors(r.key, `${r.label} é obrigatório`);
      }
      if (!isEmpty(value) && value.length < r.minLength!) {
        addErrors(r.key, `${r.label} deve conter no mínimo ${r.minLength!} caracteres`);
      }
      if (!isEmpty(value) && value.length > r.maxLength!) {
        addErrors(r.key, `${r.label} deve conter no máximo ${r.maxLength!} caracteres`);
      }
      if (!isEmpty(value) && parseInt(value, 10) < r.minValue!) {
        addErrors(r.key, `${r.label} deve ser no mínimo ${r.minValue!}`);
      }
      if (!isEmpty(value) && parseInt(value, 10) > r.maxValue!) {
        addErrors(r.key, `${r.label} deve ser no máximo ${r.maxValue!}`);
      }
      if (!isEmpty(value) && r.pattern && !r.pattern.test(value)) {
        addErrors(r.key, `${r.label} não está no formato correto`);
      }
    });
